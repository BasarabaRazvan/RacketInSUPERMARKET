...
(define (dequeue q)
  (cond
    ((= (queue-size-l q) 0) (make-queue (cdr (reverse (queue-right q))) '() (- (queue-size-r q) 1) 0))
    (else (make-queue (cdr (queue-left q)) (queue-right q) (- (queue-size-l q) 1) (queue-size-r q)))
    ))
...
(define (top q)
   (cond
    ((= (queue-size-l q) 0) (car (reverse (queue-right q))))
