...
(define (rotate left right Acc)
  (cond
    ((stream-empty? left) (stream-cons (car right) Acc))
...
(define (enqueue x q)
  (cond
    ...
    (else (make-queue (rotate (queue-left q) (cons x (queue-right q)) empty-stream) '() (+ (queue-size-l q) (queue-size-r q) 1) 0))
  ))
...
(define (dequeue q)
   (cond
    ...
    (else (make-queue (rotate (stream-rest (queue-left q)) (queue-right q) empty-stream) '() (- (+ (queue-size-l q) (queue-size-r q)) 1) 0))
  ))
...
(define (top q)
 (cond
    ((> (queue-size-l q) (queue-size-r q)) (stream-first (queue-left q)))
    (else (stream-first (rotate (queue-left q) (queue-right q) empty-stream)))
  ))
