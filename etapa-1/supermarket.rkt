...
(define (tt+ C minutes)
  (make-counter (counter-index C) (+ (match C [(counter index tt queue) tt]) minutes) (counter-queue C)
    ))
...
(define (min-tt-h counters acc-index acc-tt)
  (cond
    ((null? counters) (cons acc-index acc-tt))
    ((< (counter-tt (car counters)) acc-tt) (min-tt-h (cdr counters) (counter-index (car counters)) (counter-tt (car counters))))
    (else (min-tt-h (cdr counters) acc-index acc-tt))
    ))

(define (min-tt counters)
  (cond
    ((null? counters) (cons null null))
    (else (min-tt-h (cdr counters) (counter-index (car counters)) (counter-tt (car counters))))
    ))
...
(define (add-to-counter C name n-items)
  (make-counter (counter-index C) (+ (match C [(counter index tt queue) tt]) n-items) (append (counter-queue C) (list (cons name n-items)))
                ))
...
(define (serve requests C1 C2 C3 C4)
  ...
  ;(define (new-counters request C1 C2 C3 C4)
  ;  'a)
      ...
      (match (car requests)
        ...
        [(list name n-items)
         (cond
           ...
           ((or (= (car (min-tt (list C1 C2 C3 C4))) 2) (= (car (min-tt (list C2 C3 C4))) 2)) (serve (cdr requests) C1 (add-to-counter C2 name n-items) C3 C4))
           ((or (= (car (min-tt (list C1 C2 C3 C4))) 3) (= (car (min-tt (list C2 C3 C4))) 3)) (serve (cdr requests) C1 C2 (add-to-counter C3 name n-items) C4))
